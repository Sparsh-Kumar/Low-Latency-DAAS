...
class PostgresDatabase(BaseDatabase):
  ...
  def find_one(
    ...
    if not filter:
      filter = {}
    query = sql.SQL("SELECT * FROM {}").format(table.table_name)
    params: list[Any] = []
    if filter:
  ...
  def insert_one(
    ...
    columns = [sql.Identifier(column) for column in record.keys()]
    placeholders = [sql.Placeholder() for _ in record]
    query = sql.SQL("INSERT INTO {} ({}) VALUES ({})").format(
      table.table_name,
      sql.SQL(", ").join(columns),
      sql.SQL(", ").join(placeholders),
    )
    table.database.execute(query, list(record.values()))
  ...
  def insert_many(
    ...
    query = sql.SQL("INSERT INTO {} ({}) VALUES ({})").format(
      table.table_name, sql.SQL(", ").join(columns), sql.SQL(", ").join(placeholders)
    )
    values = [[record[column] for column in column_names] for record in records]
  ...
  def delete_many(
    ...
    query = sql.SQL("DELETE FROM {}").format(table.table_name)
    params: list[Any] = []
    if filter:
      conditions: list[sql.Composable] = []
      for column, value in filter.items():
